\chead{}
\pdfbookmark[0]{Page de titre}{titre}
\thispagestyle{empty}
\newgeometry{margin=2em}
% \newgeometry{left=3em,right=2em,top=2em,bottom=2em} %% marge pour la reliure en cas d'exemplaire imprimé

\vspace{1em}

\begin{center}
	\begin{minipage}[c]{.5\linewidth}
		\raggedright\includegraphics[height=7em]{logo_amu}
	\end{minipage}\hfill
	\begin{minipage}[c]{.5\linewidth}
		\raggedleft\includegraphics[height=7em]{logo-ibdm.png} %% logo établissement en cotutelle, le cas échéant
	\end{minipage}\hfill
\end{center}

%\vspace{.4em}

\begin{center}
	\begin{minipage}[c]{.77\linewidth}
		\textcolor{yellowamu}{\noindent\rule{\textwidth}{4pt}}
	\end{minipage}\hfill
	% \begin{minipage}[c]{.23\linewidth}
	% 	\raggedleft\textsf{NNT : 2025AIXM0000}
	% 	% renseigner votre numéro national de thèse (NNT)
	% \end{minipage}\hfill
\end{center}

%\vspace{1em}

\doublespacing
\begin{flushleft}
    \textsf{\HUGE\textcolor{blueamu}{THÈSE D'HABILITATION\\À DIRIGER LA RECHERCHE}}\\
	\textsf{\Large Soutenue à AMU ― Aix-Marseille Université}\\
	%\textsf{\Large dans le cadre d'une cotutelle avec }\\ %% établissement en cotutelle, le cas échéant
	\textsf{\Large le XX septembre 2026 par}\\
\end{flushleft}
\vspace{2em}
\begin{center}
	\textsf{\textbf{\Huge Léo Guignard}}\\
    \vspace{2em}
	\textsf{\LARGE Computer Science, Morphogenesis and Variability}\\ 
	% \textsf{\Large sous-titre de la thèse}\\
\end{center}
\singlespacing

\vspace{4em}

\begin{center}
	\begin{minipage}[t]{.45\linewidth}
    	    \vspace{.5em}
        	\textsf{\textbf{Sciences du vivant}}
        	
        	% \textsf{Liste des disciplines en \hyperref[chap:doctorats]{annexe A}}
        	
    	    \vspace{1em}
        	\textsf{\textbf{Biologie computationnelle}}
        	
        	% \textsf{Liste des spécialités en \hyperref[chap:doctorats]{annexe A}}
        	
    	    \vspace{2em}
        	\textsf{\textbf{École doctorale 658 - Sciences du Vivant}}
        	
        	% \textsf{Liste des écoles doctorales en \hyperref[chap:doctorats]{annexe A}}
        	
    	    \vspace{1em}
        	\textsf{\textbf{IBDM}}
        	
        	% \textsf{Renseigner les partenaires institutionnels\\
        	% et les partenaires privés, un par ligne
        	% }

    	    % \vspace{3em}
        	% \textsf{Consignes de présentation détaillées\\
			% des pages liminaires en \hyperref[chap:consignes]{annexe B}}
	\end{minipage}\hfill
	\begin{minipage}[t]{.03\linewidth}
	    \rule[-280pt]{1pt}{280pt}
	\end{minipage}\hfill
	\begin{minipage}[t]{.52\linewidth}
	    \vspace{.5em}
    	\textsf{\textbf{Composition du jury}}

	    \vspace{1em}
    	\textsf{
        \begin{tabular}{p{12em} p{10em}}
        	Yohanns BELLAÏCHE & Rapporteur \\
        	Institut Curie, Genetics and Developmental Biology \\
        	\\
        	Francis CORSON & Rapporteur \\
        	Physics laboratory of École Normale Supérieure \\
        	\\
        	Emmanuel FAURE & Rapporteur \\
        	Laboratoire d’Informatique, de Robotique et de Microélectronique de Montpellier \\
            \\
        	Florence HUBERT & Présidente du jury \\
        	Institut de Mathématiques de Marseille  \\
            \\
        	Pierre-François LENNE & Tuteur, Examinateur \\
        	Marseille Developmental Biology Institute\\
        %    \\
        % 	Prénom NOM & Membre invité·e \\
        % 	Titre et affiliation \\
        \end{tabular}
        }
	\end{minipage}\hfill
\end{center}       

\vspace{2em}

% \begin{center} %% logos partenaires
% 	\begin{minipage}[c]{.25\linewidth}
% 		\centering\includegraphics[height=5em]{example-image-a} 
% 	\end{minipage}\hfill
% 	\begin{minipage}[c]{.25\linewidth}
% 		\centering\includegraphics[height=5em]{example-image-b}
% 	\end{minipage}\hfill
% 	\begin{minipage}[c]{.25\linewidth}
% 		\centering\includegraphics[height=5em]{example-image-c} 
% 	\end{minipage}\hfill
% 	\begin{minipage}[c]{.25\linewidth}
% 	\end{minipage}\hfill
% \end{center}

\restoregeometry
