\chapter{A computational framework for statistical atlases}
\chaptertoc{}
...
\end{enumerate}
\section{Scientific context}\label{sec:context}
\paragraph{}The Introduction of this manuscript described how, over the past decade, light-sheet fluorescence microscopy and (spatially resolved) single-cell omics have opened the observation of embryonic development at the single-cell scale, and how the sheer size and dimensionality of the datasets they produce have made computational analysis unavoidable.
I take this situation as the starting point of my research programme, and I focus here on the two specific bottlenecks that this programme aims at removing.
\paragraph{}\underline{From individual samples to statistical representatives.}
Whatever the modality, a single acquisition documents a single individual.
Embryogenesis is reproducible, yet no two embryos are identical: cells divide at slightly different times, tissues fold at slightly different rates and expression levels fluctuate from one sample to the next.
Consequently, deciding whether a difference measured between a wild-type and a perturbed sample is a genuine effect of the perturbation, or whether it merely falls within the normal range of variation, requires more than a pair of recordings.
It requires an average representative of the normal development together with a measure of its dispersion, that is, a statistical atlas.
Such atlases remain rare~\cite{Wolff:2018aa,McDole:2018aa,Li2019,Guignard2020,Leroy2026}, and the few that exist have been built for one organism and one modality at a time.
They mostly rely on a reference sample that has to be chosen arbitrarily and on manually placed landmarks, two ingredients that bias the resulting average and that prevent scaling to cohorts of tens of samples.
\paragraph{}\underline{From separate modalities to an integrated view.}
Cell morphodynamics and cell molecular content are, today, acquired, reconstructed, stored and visualised independently, by different communities and with different tools.
Morphological measurements come as cell trajectories and lineage trees, while near single-cell spatial omics come as high-dimensional per-cell tables whose three-dimensional coherence is, moreover, lost between consecutive sections.
No common data structure, and no common visualisation environment, currently articulates the two modalities.
As a result, the questions that biologists spontaneously ask, such as \emph{which genes are expressed where and when a given morphogenetic event takes place?}, cannot yet be answered systematically.
\paragraph{}Both bottlenecks are computational before being biological: they have to be lifted before the corresponding biological questions can even be formulated quantitatively.
This is precisely what my research programme proposes to do.
\section{Objectives}\label{sec:objectives}
\paragraph{}My research programme tackles these two bottlenecks with a single objective: better understanding the interplay between cell dynamics and cell gene expression patterns, at the single-cell scale and at the scale of whole organisms.
Reaching it requires, on the one hand, algorithms that extract quantitative measurements from the acquisition methods mentioned above and aggregate them into statistical atlases and, on the other hand, data structures and interfaces that make the resulting quantitative information easy to visualise, browse and query, both for me and for the biology community.
This work will be carried out in close relationship with other biologists, computer scientists and physicists, and follows the three axes announced in the Introduction, which the remainder of this chapter details and turns into a concrete work plan:
\begin{itemize}
   \item \textbf{Axis 1} (Section~\ref{sec:morphodynamic}): \underline{Morphological}: Decipher cell dynamics by quantifying them in developing organisms from \emph{in-toto} recordings and build dynamic statistical averages of these quantifications, on \textit{P. hawaiensis}, \textit{D. melanogaster} and gastruloids
   \item \textbf{Axis 2} (Section~\ref{sec:sc3D}): \underline{Molecular}: Systematically identify genes of interest by quantifying the single-cell molecular content of developing organisms from near single-cell spatial omics and build spatial averages of these quantifications, through the sc3D and sc3D-viewer libraries
   \item \textbf{Axis 3} (Section~\ref{sec:morpholecular}): \underline{Morpholecular}: Bridge the morphological and the molecular information to understand how they interplay to control embryogenesis at the single-cell scale
\end{itemize}
\begin{figure}[h!]
   \begin{center}
...
\section{Morphodynamic atlases of embryonic development}\label{sec:morphodynamic}
...
\section{sc3D: a fast and easy dive into spatial near single-cell omics}\label{sec:sc3D}
...
\end{itemize}
In this axis, I propose to build upon two computational libraries that I have already prototyped and publicly released, sc3D and sc3D-viewer, and to turn them into a complete and generic framework to efficiently handle and analyse (sc3D) and dynamically display and browse (sc3D-viewer) scSO datasets.
In their current state, both libraries were written for, and validated on, a single use case: the mouse spatial transcriptomic atlas presented in Section~\ref{Project-3}~\cite{Sampath-Kuma:2022aa}.
sc3D reconstructs a three-dimensional dataset from its two-dimensional sections and stores it, and sc3D-viewer displays it within napari.
What follows therefore does not describe libraries to be started from scratch, but the functionalities, and the algorithmic questions that come with them, that remain to be designed and implemented to extend these prototypes beyond that single use case.
These two libraries split the axis in two sub-axes:
\begin{itemize}
   \item \textbf{Sub-axis 2.1: sc3D}.
   sc3D extends the already existing AnnData~\cite{Wolf2018} library, allowing handling of classic single-cell transcriptomic datasets.
   I will extend it further so that it organises the spatial (position and neighbourhood relationship) information in efficient data structures, and so that it hosts the scSO specific preprocessing and analysis methods that are currently missing.
   \item \textbf{Sub-axis 2.2: sc3D-viewer}.
   sc3D-viewer is based on the sc3D data structure and already allows the display of scSO datasets in 3D.
   I will extend it to project, on the fly, the expression and/or co-expression of measured genes or any post-computed metrics, and to act as a graphical front-end to the methods of sc3D.
\end{itemize}
\paragraph{}\underline{Where the scSO datasets come from.}
These developments are driven by, and will be validated on, two sources of scSO data.
The first one is already available: the mouse embryo datasets acquired with SlideSeqV2~\cite{Stickels2021} in collaboration with the groups of F. Chen and A. Meissner (Section~\ref{sec:sc3D-community}).
The second one still has to be produced: the scSO datasets of \textit{P. hawaiensis} embryos that will be acquired together with A. Pavlopoulos.
Because these embryos will be sequenced at the end of a light-sheet recording of the very same sample, so as to obtain both modalities from a single individual, the design of their acquisition is inseparable from the multimodal effort and is therefore described with it, in Section~\ref{sec:morpholecular}.
The present axis is where the computational tools that these acquisitions will feed, and that Axis 3 will in turn reuse, are built: the two sections should be read as the software side (Section~\ref{sec:sc3D}) and the experimental side (Section~\ref{sec:morpholecular}) of one and the same scSO effort.
\begin{figure}[!th]
   \begin{center}
...
\paragraph{}To that aim, I will pursue the development of sc3D-viewer, the 3D viewer for scSO that I have already prototyped and released (\href{https://www.github.com/guignardlab/napari-sc3D-viewer}{napari-sc3D-viewer}) and that is built on napari, a software initially dedicated to 3D visualisation of microscopy images.
The current prototype loads a reconstructed dataset, displays it in 3D and projects onto the cells the expression of a chosen gene (Fig.~\ref{fig:sc3D-viewer}).
Beyond this, sc3D-viewer will become not only a way of exploring scSO datasets but also a graphical interface for sc3D, allowing users to run the tools and methods proposed in sc3D without having to write code.
\paragraph{}The functionalities that remain to be developed all aim at making the viewer not only a display but an intuitively interactive one.
...
\paragraph{}The response times already obtained with the prototype, on datasets of tens of thousands of cells carrying tens of thousands of measurements each, convince me that the rapid, seamless and in depth user experience that is required to explore such dense and large datasets is indeed within reach.
\subsection{Usefulness to the biological community in general}\label{sec:sc3D-community}
...
\section{Morpholecular atlases}\label{sec:morpholecular}
...
This acquisition is the production step announced in Section~\ref{sec:sc3D}: the scSO datasets that it generates are precisely those that the sc3D framework of Axis 2 will reconstruct, analyse and display, and it is described here rather than there because it cannot be designed independently from the imaging it is paired with.
